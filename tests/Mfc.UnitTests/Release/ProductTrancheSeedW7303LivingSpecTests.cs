...
public sealed class ProductTrancheSeedW7303LivingSpecTests
{
    ...
    public void Ac1KnownLimitationsAndQueueSeedPlan40AfterPlan39Complete()
    {
        ...
        Assert.Contains(
            "W7-304 | [#1015](https://github.com/sesquicadaver/MTDirector/issues/1015) | PLAN-40 — Inventory Controller host journald/syslog identity (SyslogIdentifier) | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        Assert.Contains(
            "W7-305 | [#1016](https://github.com/sesquicadaver/MTDirector/issues/1016) | Seed first PLAN-40 atomic row after inventory → OPS-HOST-LOG-01 | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        Assert.Contains("§3.C NEXT = W7-304 (#1015)", roadmap, StringComparison.Ordinal);
        ...
        Assert.Contains("§3.C NEXT = W7-304 (#1015)", plan39, StringComparison.Ordinal);
        ...
        Assert.Contains("§3.C NEXT = W7-304 (#1015)", plan, StringComparison.Ordinal);
        ...
        Assert.Contains("Inventory **OPEN**", plan40, StringComparison.Ordinal);
        Assert.Contains("W7-304", plan40, StringComparison.Ordinal);
        Assert.Contains("W7-305", plan40, StringComparison.Ordinal);
        Assert.Contains("§3.C NEXT = W7-304 (#1015)", plan40, StringComparison.Ordinal);
        Assert.Contains("SyslogIdentifier", plan40, StringComparison.Ordinal);
        Assert.Contains("c2a807e9", plan40, StringComparison.Ordinal);
