...
public sealed class ProductTrancheSeedW7311LivingSpecTests
{
    ...
    public void Ac1KnownLimitationsAndQueueSeedPlan42AfterPlan41Complete()
    {
        ...
        Assert.Contains(
            "W7-312 | [#1031](https://github.com/sesquicadaver/MTDirector/issues/1031) | PLAN-42 — Inventory Controller HTTP liveness/readiness probes beyond gRPC health | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains("Inventory **OPEN**", plan42, StringComparison.Ordinal);
        Assert.Contains("W7-312", plan42, StringComparison.Ordinal);
        Assert.Contains("W7-313", plan42, StringComparison.Ordinal);
        Assert.Contains("§3.C NEXT = W7-312 (#1031)", plan42, StringComparison.Ordinal);
        Assert.Contains("MapGrpcHealthChecksService", plan42, StringComparison.Ordinal);
        Assert.Contains("1e0c475c", plan42, StringComparison.Ordinal);
