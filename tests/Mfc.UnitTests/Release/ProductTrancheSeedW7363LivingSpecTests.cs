...
public sealed class ProductTrancheSeedW7363LivingSpecTests
{
    ...
    public void Ac1KnownLimitationsAndQueueSeedPlan55AfterPlan54Complete()
    {
        ...
        Assert.Contains(
            "W7-364 | [#1135](https://github.com/sesquicadaver/MTDirector/issues/1135) | PLAN-55 — Inventory capture progress fault correlation | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        Assert.Contains(
            "W7-365 | [#1136](https://github.com/sesquicadaver/MTDirector/issues/1136) | Seed first PLAN-55 atomic row after inventory → SNAP-FAULT-CORR-01 | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        ...
        Assert.Contains("§3.C NEXT = W7-364 (#1135)", plan55, StringComparison.Ordinal);

        Assert.Equal(4, Count(snapshot, "CorrelationId = ProtoUuid.FromGuid(Guid.NewGuid())"));
        Assert.Contains("ToRpcException(result.Error!)", snapshot, StringComparison.Ordinal);
