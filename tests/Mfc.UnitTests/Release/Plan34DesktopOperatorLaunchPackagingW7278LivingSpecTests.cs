...
public sealed class Plan34DesktopOperatorLaunchPackagingW7278LivingSpecTests
{
    ...
    public void Ac1Plan34InventoryDocumentsRankedRowsAndSeedsDeskHostLinux01()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "W7-280 | [#966](https://github.com/sesquicadaver/MTDirector/issues/966) | DESK-HOST-LINUX-01 — freedesktop .desktop template for framework-dependent Desktop | **OPEN**",
            roadmap,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Empty(Directory.GetFiles(Path.Combine(root, "packaging"), "*.desktop", SearchOption.AllDirectories));
        Assert.False(File.Exists(Path.Combine(root, "packaging/windows/mfc-desktop-start-menu.ps1")));
    }
