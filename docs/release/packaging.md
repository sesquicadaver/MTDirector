...
| Template | Path | Notes |
|----------|------|-------|
| systemd (Linux) | [`../../packaging/systemd/mfc-controller.service`](../../packaging/systemd/mfc-controller.service) | OPS-HOST-SYSTEMD-01 — framework-dependent Controller; `WorkingDirectory`/`ExecStart` → `/opt/mfc/controller/Mfc.Controller`; also copied into `$OUT_DIR/controller/` by `package-controller.sh` (OPS-HOST-BUNDLE-01). OPS-HOST-LOG-01 — `SyslogIdentifier=mfc-controller` + `StandardOutput=journal` / `StandardError=journal` (bundled copy inherits) |
