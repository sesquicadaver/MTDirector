...
- Live physical CRS hardware exercise is **OFF**. Scripted CRS fixture + `VrrpCrsE2ELivingSpecTests` AC11 are the DoD substitute. Physical CRS is **ops residual**, not a §3 stop-gate. **§3.C NEXT = SEC-14 (#396)** after SEC-13.
...
- **SEC-07…SEC-13 DONE:** Zone/policy (SEC-07), `UpdateConnectionProfileUseCase` + `DeploymentWorkflowUseCases` (SEC-08), `OnboardingWorkflowUseCases` (SEC-09), `ExpireIncidentDenyOverlayBindingUseCase` (SEC-10), `DetectManagedDriftUseCase` + `EmitResponseFeedbackUseCase` store+audit (SEC-11), `CaptureSnapshotUseCase` persist+audit (SEC-12), and `UpsertDeviceHashStateUseCase` (SEC-13) share `IUnitOfWork` for entity+idempotency+audit co-writes. Feedback **delivery** and RouterOS capture remain outside the DB boundary. Intentional residual: resolve-only zone updates (no idempotency/audit triple); Start* pre-runtime `AddOperationAsync` stays outside UoW; `OpenEndpointPresenceUseCase` multi-store writes (candidate SEC-14).
