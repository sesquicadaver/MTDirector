...
- **SEC-07 / SEC-08 DONE:** Application zone/policy mutation paths (SEC-07) and `UpdateConnectionProfileUseCase` plus `DeploymentWorkflowUseCases` plan/start/rollback terminal persists (SEC-08) share `IUnitOfWork`. Intentional residual: resolve-only zone updates (no idempotency/audit triple); `StartDeployment` pre-runtime `AddOperationAsync` stays outside UoW so RouterOS work is not held inside a DB transaction.
