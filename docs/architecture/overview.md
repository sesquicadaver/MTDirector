...
| Desktop MVP workflows | Done (M6-04) + W1.1–W1.5 | Seven modules + `DriftService`/`AuditService` read paths; Shell nav; no auto-fix; Living Spec AC 1–12 (+ AC#2b Add router); drift findings bound from list; shared `mfc-*` shell styles + wrap toolbars (overlap/readability polish 2026-08-29) |
