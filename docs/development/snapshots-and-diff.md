...
- W3.1: Capture button → `StartCapture` + `WatchCapture` (device_id); progress shows stage / `current_section` and, on failure, the shared correlation id (`SNAP-FAULT-CORR-01`). A `Failed` stage sets shell `ErrorText` to that same `FormatCaptureProgress` line (`SNAP-ERRTEXT-CORR-01`). COMPLETED reloads the device list. Not a Desktop→RouterOS write and not WriteEnabled.
- DESK-SVC-FAULT-01: snapshot viewer load, snapshot diff load/compare, and inventory refresh set service `Error` from `DesktopRpcFaultText.Format` on `RpcException` (code + correlation id). Non-RPC exceptions keep `Exception.Message`.
