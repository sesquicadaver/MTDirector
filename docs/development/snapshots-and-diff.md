...
- W3.1: Capture button → `StartCapture` + `WatchCapture` (device_id); progress shows stage / `current_section` and, on failure, the shared correlation id (`SNAP-FAULT-CORR-01`); COMPLETED reloads the device list. Not a Desktop→RouterOS write and not WriteEnabled.
