...
- Connect and reconnect catches that set `Disconnected` use `DesktopRpcFaultText.Format` when the failure is an `RpcException` (DESK-CONN-DISC-01), so shell status includes the code and correlation id. Non-RPC exceptions keep `Exception.Message`. `TlsError` stays `ex.Message`. The static `Health check timed out.` sentence is unchanged.
...
- A Snapshot capture that ends in `CaptureStage.Failed` sets shell `ErrorText` to `SnapshotViewerViewModel.FormatCaptureProgress` (SNAP-ERRTEXT-CORR-01), the same line already shown as capture progress, including `(correlation {id})` when the progress `ErrorDetail` has a 16-byte id. The capture `RpcException` path stays `DesktopRpcFaultText`.
- Snapshot viewer load, snapshot diff load/compare, and inventory refresh set service `Error` from `DesktopRpcFaultText.Format` when the failure is an `RpcException` (DESK-SVC-FAULT-01), so shell `ErrorText` includes the code and correlation id. Non-RPC exceptions keep `Exception.Message`.
