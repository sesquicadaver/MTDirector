...
Endpoint comes from `src/Mfc.Desktop/appsettings.json` (`Desktop:ControllerEndpoint`). Unary Controller RPCs time out after `Desktop:UnaryCallTimeoutSeconds` (default **30**, fail-closed when ≤0). Watch streams are not deadline-bounded. Operator `ErrorText` includes the Controller error code and correlation id when `mfc-error-detail-bin` is present (DESK-RPC-FAULT-01). Connect/reconnect `AuthenticationFailed` shell status uses the same text (DESK-CONN-FAULT-01). A failed VRRP pair consistency RPC shows that same text on the pair status line (DESK-VRRP-FAULT-01). A VRRP member capture that reports `ErrorDetail` shows the Snapshots `(correlation {id})` suffix on that pair status line, including an incomplete capture (DESK-VRRP-PROG-01). Drift, Audit, Incident, Routing assurance, and GetNodeWorkflow RPC failures show that same `DesktopRpcFaultText` on the panel status line (DESK-PANEL-FAULT-01). A Snapshot capture that ends `Failed` sets shell `ErrorText` to `FormatCaptureProgress`, including `(correlation {id})` when the progress detail has one (SNAP-ERRTEXT-CORR-01). The capture `RpcException` path stays `DesktopRpcFaultText`. Snapshot viewer, snapshot diff, and inventory refresh do the same for service `Error` when the caught exception is an `RpcException` (DESK-SVC-FAULT-01); other exceptions keep `Exception.Message`. Connect and reconnect `Disconnected` catches do the same (DESK-CONN-DISC-01). `AuthenticationFailed` stays DESK-CONN-FAULT-01. `TlsError` stays `ex.Message`. The static health-timeout sentence is unchanged.
