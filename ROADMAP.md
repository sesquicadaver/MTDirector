...
**Базовий коміт аудиту:** M5-03 — management guard verification DONE; черга зсунута на M5-04
...
| M3 Compiler | 8 | 0 | 100% |
| M5 Onboarding | 3 | 7 | 30% |
| M4 Safe deploy | 0 | 13 | 0% |
| M6 E2E / drift | 0 | 9 | 0% |
| M7 Post-MVP | 0 | 27 | 0% |
| **Разом** | **77** | **59** | **57% issues** |

MVP issues (109) = **77 done + 32 remaining** до MVP CLOSED (**71%**).  
N1-06/N1-07 входять у N1 Open, не в M4/M6. Post-MVP M7 = **27** лише після M6-09.  
Операційно: read-only зріз **готовий**; policy authoring Desktop **готовий**; **M3 Compiler CLOSED**; onboarding domain/persistence **готовий**; prerequisites **готові**; management guard verification **готовий**; anchor plan / apply / drift = далі по черзі (NEXT M5-04).
...
| `Mfc.Domain.Onboarding` | immutable plans + plan hasher + operation SM + write-ahead steps + bootstrap artifact + `ManagementState` (M5-01) + prerequisite validator (M5-02) + `GuardProfile` / guard verifier (M5-03) |

**NEXT = M5-04:** [M5-04](https://github.com/sesquicadaver/MTDirector/issues/79) Implement explicit permanent anchor placement (після M5-03 #78). Desktop stays Contracts-only (ADR 0005).
...
**Кінець черги:** 59 відкритих атомарних задач (32 до MVP CLOSED + 27 M7). Start here: #79 M5-04.
...
|---------|--------:|----------|
| До MVP CLOSED | 32 | M5-04…M6-09 + N1-06/07 |
| Post-MVP M7 | 27 | лише після M6-09 |
| **Нереалізовано разом** | **59** | 32 MVP + 27 M7 |
| DONE у коді (§2.2) | 77 | M0+M1+N1-01…05+M2-01…18+M3-01…08+M5-01…03 |
...
14. Відкрити **M5-04** → [issue #79](https://github.com/sesquicadaver/MTDirector/issues/79).
15. Після merge — закреслити рядок у §3 (або перенести в §2.2 DONE) і взяти наступний `#`.
16. Не стартувати M4, доки не закрито **M5-10** (issue #85).
17. Не стартувати M7, доки не закрито **M6-09** (черга #95).
