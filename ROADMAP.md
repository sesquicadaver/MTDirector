...
**Базовий коміт аудиту:** M2-07 — M1 CLOSED; черга зсунута на M2-08
...
| `Mfc.Application` | inventory/snapshot + policy draft/rule CRUD + compose-on-read + address/service/zone evaluators + N1-05 snapshot topology enrichment |
| `Mfc.Controller` | health + `InventoryService` + `SnapshotService` + `ZoneService` + `PolicyService` (incl. compose) gRPC |
...
| `Mfc.Domain.Policy` | lifecycle + Pipeline v1 + chain contracts + address/service/zone + N1-05 marker expand + typed rules + logical compose (M2-07) |

**NEXT = M2-08:** [M2-08](https://github.com/sesquicadaver/MTDirector/issues/55) temporary deny-stage exceptions (після M2-07 #54 DONE).
...
1. Відкрити **M2-08** → [issue #55](https://github.com/sesquicadaver/MTDirector/issues/55) (M2-07 #54 DONE).
2. Після merge — закреслити рядок у §3 (або перенести в §2.2 DONE) і взяти наступний `#`.
3. Не стартувати M2, доки не закрито **M1-34** (черга #33) — **DONE**; M2-01…M2-07 + N1-05 — **DONE**.
