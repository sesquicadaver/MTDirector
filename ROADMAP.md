...
**Базовий коміт аудиту:** M5-06 — scheduler proof + onboarding watchdog DONE; черга зсунута на M5-07
...
| `Mfc.Domain.Onboarding` | immutable plans + plan hasher + operation SM + write-ahead steps + bootstrap artifact + `ManagementState` (M5-01) + prerequisite validator (M5-02) + `GuardProfile` / guard verifier (M5-03) + `AnchorPlacementPlanner` (M5-04) + `OnboardingBootstrapWritePlanner` (M5-05) + `OnboardingWatchdogPlanner` (M5-06) |

**NEXT = M5-07:** [M5-07](https://github.com/sesquicadaver/MTDirector/issues/82) Implement onboarding execution and verification (після M5-06 #81). Desktop stays Contracts-only (ADR 0005).
...
**Кінець черги:** 56 відкритих атомарних задач (29 до MVP CLOSED + 27 M7). Start here: #82 M5-07.
...
|---------|--------:|----------|
| До MVP CLOSED | 29 | M5-07…M6-09 + N1-06/07 |
| Post-MVP M7 | 27 | лише після M6-09 |
| **Нереалізовано разом** | **56** | 29 MVP + 27 M7 |
| DONE у коді (§2.2) | 80 | M0+M1+N1-01…05+M2-01…18+M3-01…08+M5-01…06 |
...
17. Відкрити **M5-07** → [issue #82](https://github.com/sesquicadaver/MTDirector/issues/82).
18. Після merge — закреслити рядок у §3 (або перенести в §2.2 DONE) і взяти наступний `#`.
19. Не стартувати M4, доки не закрито **M5-10** (issue #85).
20. Не стартувати M7, доки не закрито **M6-09** (черга #95).
